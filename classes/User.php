<?php
    ...
    class User extends Database{
        ...
        public function login($request){
            #Received the data from the form
            $username = $request['username'];
            $password = $request['password'];
            ...
            if ($result->num_rows == 1) {
                ...
                $user = $result->fetch_assoc(); // if we search for Mary Watson
                //$user = ['id'=> 1, 'first_name' => 'Mary', 'last_name' => 'Watson', 'username' => 'marywatson', 'db_password' => '$2y$10$Fts5/5F6Hy8.2Pg1T6T7B.dCC2ls1AyDGURT'];

                if (password_verify($password, $user['db_password'])) {
                    # If the password is matched, then create a SESSION VARIABLES for future use
                    session_start();
                    $_SESSION['id'] = $user['id'];
        ...
        public function update($request, $files){
            ...
            $last_name      = $request['last_name'];
            $username       = $request['username'];
            $photo          = $files['photo']['name']; //2 dimensional array [][]
            $tmp_photo      = $files['photo']['tmp_name']; // tmp_name ---> the file is placed in a temporary storage/folder before moving it to the destination
            ...
            if ($this->conn->query($sql)) {
                ...
                # If there is an uploaded photo, save it to the Db and save the file to
                # images folder
